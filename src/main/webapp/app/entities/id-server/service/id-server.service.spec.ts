...
import { sampleWithRequiredData, sampleWithNewData, sampleWithPartialData, sampleWithFullData } from '../id-server.test-samples';

import { IdServerService } from './id-server.service';

const requireRestSample: IIdServer = {
...
describe('SampleType Service', () => {
  ...
  let expectedResult: IIdServer | IIdServer[] | boolean | null;
  ...
  describe('Service methods', () => {
    ...
    describe('addSampleTypeToCollectionIfMissing', () => {
      it('should add a SampleType to an empty array', () => {
        const sampleType: IIdServer = sampleWithRequiredData;
      ...
      it('should not add a SampleType to an array that contains it', () => {
        const sampleType: IIdServer = sampleWithRequiredData;
        const sampleTypeCollection: IIdServer[] = [
          {
            ...sampleType,
          },
      ...
      it("should add a SampleType to an array that doesn't contain it", () => {
        const sampleType: IIdServer = sampleWithRequiredData;
        const sampleTypeCollection: IIdServer[] = [sampleWithPartialData];
        expectedResult = service.addSampleTypeToCollectionIfMissing(sampleTypeCollection, sampleType);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(sampleType);
      });
      ...
      it('should add only unique SampleType to an array', () => {
        const sampleTypeArray: IIdServer[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const sampleTypeCollection: IIdServer[] = [sampleWithRequiredData];
      ...
      it('should accept varargs', () => {
        const sampleType: IIdServer = sampleWithRequiredData;
        const sampleType2: IIdServer = sampleWithPartialData;
