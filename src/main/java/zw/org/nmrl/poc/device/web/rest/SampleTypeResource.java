...
import zw.org.nmrl.poc.device.service.SampleTypeQueryService;
...
/**
 * REST controller for managing {@link zw.org.nmrl.poc.device.domain.SampleType}.
 */
@RestController
@RequestMapping("/api")
public class SampleTypeResource {
    ...
    private final SampleTypeQueryService sampleTypeQueryService;

    public SampleTypeResource(
        SampleTypeService sampleTypeService,
        SampleTypeRepository sampleTypeRepository,
        SampleTypeQueryService sampleTypeQueryService
    ) {
        this.sampleTypeService = sampleTypeService;
        this.sampleTypeRepository = sampleTypeRepository;
        this.sampleTypeQueryService = sampleTypeQueryService;
    }
    ...
    public ResponseEntity<SampleType> updateSampleType(
        ...
        SampleType result = sampleTypeService.update(sampleType);
        return ResponseEntity
            .ok()
    ...
    public ResponseEntity<List<SampleType>> getAllSampleTypes(
        SampleTypeCriteria criteria,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to get SampleTypes by criteria: {}", criteria);
        Page<SampleType> page = sampleTypeQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /sample-types/count} : count all the sampleTypes.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/sample-types/count")
    public ResponseEntity<Long> countSampleTypes(SampleTypeCriteria criteria) {
        log.debug("REST request to count SampleTypes by criteria: {}", criteria);
        return ResponseEntity.ok().body(sampleTypeQueryService.countByCriteria(criteria));
    }
